 ...
 * }
   ...
   * getName(obj) === true
   */
  const propEq = curry(function (name, val, obj) { return obj && obj[name] === val })
  /**
 ...
 * }
  ...
  function arrayTuples () {
    ...
    const maxArrayLen = args.reduce(function (min, arr) { return Math.min(arr.length, min) }, 9007199254740991) // aka 2^53 âˆ’ 1 aka Number.MAX_SAFE_INTEGER
    let i; const result = []
  ...
  const RegisteredHook = /** @class */ (function () {
    ...
    RegisteredHook.prototype._matchingNodes = function (nodes, criterion) {
      ...
      const matching = nodes.filter(function (node) { return matchState(node.state, criterion) })
 ...
 * }
  ...
  function getType (cfg, urlType, location, id, paramTypes) {
    ...
    if (!cfg.type) {
      const type = location === exports.DefType.CONFIG
        ? 'any'
  ...
  const PathUtils = /** @class */ (function () {
    ...
    PathUtils.applyViewConfigs = function ($view, path, states) {
      ...
      path.filter(function (node) { return inArray(states, node.state) }).forEach(function (node) {
        ...
        const subPath = PathUtils.subPath(path, function (n) { return n === node })
        const viewConfigs = viewDecls.map(function (view) { return $view.createViewConfig(subPath, view) })
    ...
    PathUtils.inheritParams = function (fromPath, toPath, toKeys) {
      ...
      const noInherit = fromPath.map(function (node) { return node.paramSchema })
        .reduce(unnestR, [])
  ...
  const ResolveContext = /** @class */ (function () {
    ...
    ResolveContext.prototype.getResolvable = function (token) {
      const matching = this._path.map(function (node) { return node.resolvables })
        .reduce(unnestR, [])
    ...
    ResolveContext.prototype.addResolvables = function (newResolvables, state) {
      ...
      const keys = newResolvables.map(function (r) { return r.token })
      ...
      const promises = this._path.reduce(function (acc, node) {
        ...
        const getResult = function (r) {
          return r.get(subContext, trans)
            .then(function (value) { return ({ token: r.token, value }) })
        }
    ...
    ResolveContext.prototype.getDependencies = function (resolvable) {
      ...
      const subPath = PathUtils.subPath(this._path, function (x) { return x === node }) || this._path
      const availableResolvables = subPath
        .reduce(function (acc, _node) { return acc.concat(_node.resolvables) }, []) // all of subpath's resolvables
        .filter(function (res) { return res !== resolvable }) // filter out the `resolvable` argument
      const getDependency = function (token) {
        const matching = availableResolvables.filter(function (r) { return r.token === token })
  ...
  const Transition = /** @class */ (function () {
    ...
    Transition.prototype.applyViewConfigs = function (router) {
      const enteringStates = this._treeChanges.entering.map(function (node) { return node.state })
    ...
    Transition.prototype.addResolvable = function (resolvable, state) {
      ...
      const targetNode = find(topath, function (node) { return node.state.name === stateName })
    ...
    Transition.prototype._changedParams = function () {
      ...
      const nodeSchemas = tc.to.map(function (node) { return node.paramSchema })
      const _a = [tc.to, tc.from].map(function (path) { return path.map(function (x) { return x.paramValues }) }); const toValues = _a[0]; const fromValues = _a[1]
    ...
    Transition.prototype.error = function () {
      ...
      const invalidParams = paramDefs.filter(function (param) { return !param.validates(values$$1[param.id]) })
 ...
 * }
  ...
  function resolvablesBuilder (state) {
    ...
    const objects2Tuples = function (resolveObj, resolvePolicies) {
      return Object.keys(resolveObj || {}).map(function (token) { return ({ token, val: resolveObj[token], deps: undefined, policy: resolvePolicies[token] }) })
    }
  ...
  const StateBuilder = /** @class */ (function () {
    ...
    StateBuilder.prototype.build = function (state) {
      ...
      for (const key in builders) {
        ...
        const chain = builders[key].reduce(function (parentFn, step) { return function (_state) { return step(_state, parentFn) } }, noop$1)
  ...
  const StateRegistry = /** @class */ (function () {
    ...
    StateRegistry.prototype._deregisterTree = function (state) {
      const _this = this
      const all$$1 = this.get().map(function (s) { return s.$$state() })
      var getChildren = function (states) {
        const children = all$$1.filter(function (s) { return states.indexOf(s.parent) !== -1 })
  ...
  const UrlMatcher = /** @class */ (function () {
    ...
    UrlMatcher.prototype.exec = function (path, search, hash, options) {
      ...
      const allParams = this.parameters(); const pathParams = allParams.filter(function (param) { return !param.isSearch() }); const searchParams = allParams.filter(function (param) { return param.isSearch() }); const nPathSegments = this._cache.path.map(function (urlm) { return urlm._segments.length - 1 }).reduce(function (a, x) { return a + x }); const values$$1 = {}
    ...
    UrlMatcher.prototype.validates = function (params) {
      ...
      const paramSchema = this.parameters().filter(function (paramDef) { return params.hasOwnProperty(paramDef.id) })
    ...
    UrlMatcher.pathSegmentsAndParams = function (matcher) {
      ...
      const pathParams = matcher._params.filter(function (p) { return p.location === exports.DefType.PATH })
  ...
  const UrlRuleFactory = /** @class */ (function () {
    ...
    UrlRuleFactory.prototype.fromUrlMatcher = function (urlMatcher, handler) {
      ...
      function matchPriority (params) {
        const optional = urlMatcher.parameters().filter(function (param) { return param.isOptional })
        if (!optional.length) { return 0.000001 }
        const matched = optional.filter(function (param) { return params[param.id] })
  ...
  const UrlRouter = /** @class */ (function () {
    ...
    UrlRouter.prototype.stableSort = function (arr, compareFn) {
      const arrOfWrapper = arr.map(function (elem, idx) { return ({ elem, idx }) })
  ...
  const ViewService = /** @class */ (function () {
    ...
    ViewService.prototype.sync = function () {
      const _this = this
      const uiViewsByFqn = this._uiViews.map(function (uiv) { return [uiv.fqn, uiv] }).reduce(applyPairs, {})
      ...
      const depthCompare = curry(function (depthFn, posNeg, left, right) { return posNeg * (depthFn(left) - depthFn(right)) })
    ...
    ViewService.normalizeUIViewTarget = function (context, rawViewName) {
      ...
      if (relativeMatch.exec(uiViewContextAnchor)) {
        const anchor = uiViewContextAnchor.split('.').reduce(function (anchor, x) { return anchor.parent }, context)
    ...
    all: function (promises) {
      ...
      if (isObject(promises)) {
        ...
        const chain = Object.keys(promises)
          .map(function (key) { return promises[key].then(function (val) { return ({ key, val }) }) })
 ...
 * }]
  ...
  var $injector = {
    ...
    invoke: function (fn, context, locals) {
      ...
      const ensureExist = assertPredicate(function (key) { return all.hasOwnProperty(key) }, function (key) { return "DI can't find injectable: '" + key + "'" })
      const args = params.filter(ensureExist).map(function (x) { return all[x] })
    ...
    BaseLocationServices.prototype.url = function (url, replace) {
      ...
      if (isDefined(url) && url !== this._get()) {
        ...
        if (this.fireAfterUpdate) {
          this._listeners.forEach(function (cb) { return cb({ url }) })
        }
      }
  ...
  function servicesPlugin (router) {
    ...
    services.$q = $q
    return { name: 'vanilla.services', $q, $injector, dispose: function () { return null } }
  }
  ...
  var Ng1ViewConfig = /** @class */ (function () {
    ...
    Ng1ViewConfig.prototype.load = function () {
      ...
      const params = this.path.reduce(function (acc, node) { return extend(acc, node.paramValues) }, {})
      const promises = {
  ...
  const TemplateFactory = /** @class */ (function () {
    ...
    TemplateFactory.prototype.fromConfig = function (config, params, context) {
      ...
      const asTemplate = function (result) { return services.$q.when(result).then(function (str) { return ({ template: str }) }) }
      const asComponent = function (result) { return services.$q.when(result).then(function (str) { return ({ component: str }) }) }
    ...
    function $StateRefDynamicDirective ($uiRouter, $timeout) {
      ...
      return {
        ...
        link: function (scope, element, attrs, uiSrefActive) {
          ...
          const watchDeregFns = inputAttrs.reduce(function (acc, attr) { return (acc[attr] = noop$1, acc) }, {})
          function update () {
            const def = getDef()
            if (unlinkInfoFn) { unlinkInfoFn() }
            if (active) { unlinkInfoFn = active.$$addStateInfo(def.uiState, def.uiStateParams) }
        ...
        controller: ['$scope', '$element', '$attrs',
          function ($scope, $element, $attrs) {
            ...
            function update () {
              ...
              const fuzzyClasses = getClasses(states.filter(function (x) { return $state.includes(x.state.name, x.params) }))
              const exactlyMatchesAny = !!states.filter(function (x) { return $state.is(x.state.name, x.params) }).length
              ...
              const removeClasses = allClasses.filter(function (cls) { return !inArray(addClasses, cls) })
      ...
      var directive = {
        ...
        compile: function (tElement, tAttrs, $transclude) {
          return function (scope, $element, attrs) {
            let previousEl; let currentEl; let currentScope; let unregister; const onloadExp = attrs.onload || ''; const autoScrollExp = attrs.autoscroll; const renderer = getRenderer(attrs, scope); let viewConfig; const inherited = $element.inheritedData('$uiView') || rootData; const name = $interpolate(attrs.uiView || attrs.name || '')(scope) || '$default'
            const activeUIView = {
              $type: 'ng1',
              id: directive.count++,
              name,
  ...
  function registerControllerCallbacks ($q, $transitions, controllerInstance, $scope, cfg) {
    ...
    if (isFunction(controllerInstance.uiOnParamsChanged)) {
      ...
      const paramsUpdated = function ($transition$) {
        ...
        const toSchema = $transition$.treeChanges().to.map(function (node) { return node.paramSchema }).reduce(unnestR, [])
        const fromSchema = $transition$.treeChanges().from.map(function (node) { return node.paramSchema }).reduce(unnestR, [])
        ...
        if (changedToParams.length) {
          const changedKeys_1 = changedToParams.map(function (x) { return x.id })
          // Filter the params to only changed/new to params.  `$transition$.params()` may be used to get all params.
          const newValues = filter(toParams, function (val, key) { return changedKeys_1.indexOf(key) !== -1 })
